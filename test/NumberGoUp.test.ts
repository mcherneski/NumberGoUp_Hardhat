...
describe("NumberGoUp Tests", function () {
    ...
    describe("Batch Burning", function() {
        it("Should handle large NFT burns in batches when adding exempt status", async function() {
            ...
            const initialBalance = parseUnits("5000");
            await numberGoUp.transfer(nonExempt1.address, initialBalance);
            
            // Wait for initial NFTs to be minted through pending claims
            while ((await numberGoUp.pendingNFTs(nonExempt1.address)) > 0) {
                await numberGoUp.connect(nonExempt1).mintPendingNFTs();
            }

            // Verify initial state
            expect(await numberGoUp.erc721BalanceOf(nonExempt1.address)).to.equal(5005);
            console.log("Initial NFT balance:", await numberGoUp.erc721BalanceOf(nonExempt1.address));

            // Set user to exempt - this should trigger batch burns
            await numberGoUp.connect(owner).setERC721TransferExempt(nonExempt1.address, true);

            // Check immediate burn amount (should be 1000 due to batch size)
            const remainingNFTs = await numberGoUp.erc721BalanceOf(nonExempt1.address);
            console.log("NFTs remaining after initial burn:", remainingNFTs);
            
            // Check pending burns
            const pendingBurns = await numberGoUp.pendingBurns(nonExempt1.address);
            console.log("Pending burns:", pendingBurns);

            // Process remaining burns
            while ((await numberGoUp.pendingBurns(nonExempt1.address)) > 0) {
                await numberGoUp.connect(nonExempt1).burnPendingNFTs();
                console.log("Remaining pending burns:", await numberGoUp.pendingBurns(nonExempt1.address));
            }

            // Verify final state
            expect(await numberGoUp.erc721BalanceOf(nonExempt1.address)).to.equal(0);
            expect(await numberGoUp.pendingBurns(nonExempt1.address)).to.equal(0);
        });

        it("Should revert when trying to burn with no pending burns", async function() {
            await expect(numberGoUp.connect(nonExempt1).burnPendingNFTs())
                .to.be.revertedWithCustomError(numberGoUp, "NoPendingBurns");
        });
        ...
        it("Should handle pending burns correctly during transfers", async function() {
            ...
            const transferAmount = parseUnits("2500");
            await numberGoUp.transfer(exempt1.address, transferAmount);
            await numberGoUp.connect(exempt1).transfer(nonExempt1.address, transferAmount);
            ...
            expect(await numberGoUp.erc721BalanceOf(nonExempt1.address)).to.equal(2505);
